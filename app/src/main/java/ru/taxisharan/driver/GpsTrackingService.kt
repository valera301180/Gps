...
class GpsTrackingService : Service() {
    ...
    private val gson = com.google.gson.Gson() // Можно заменить на ручной JSONObject для экономии зависимостей
    ...
    private fun startLocationUpdates(interval: Long) {
        ...
        try {
            ...
            // Если разрешения отозвали, останавливаем
            stopSelf()
        }
    }
    ...
    private fun sendLocation(location: Location) {
        ...
        val json = JSONObject().apply {            put("driver_id", driverId)
        ...
        if (isNetworkAvailable()) {
            ...
            // Кэшируем при обрыве связи (макс 1000 точек)
    ...
    private fun processPendingUpdates() {
        ...
        while (iterator.hasNext()) {
            ...
            if (isNetworkAvailable()) {
                sendToServer(json)
                iterator.remove() // Удаляем из очереди только при успехе (упрощенно)
            } else {
                break
            }
        }
